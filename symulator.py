...
def uruchom_symulacje():
    try:
        ...
        # Nagłówek raportu
        text_wynik.insert(tk.END, f"RAPORT TAKTYCZNY FREECIV\n")
        ...
        btn_zapisz.config(state=tk.NORMAL) # Odblokowanie przycisku zapisu
...
def zapisz_raport():
    zawartosc = text_wynik.get("1.0", tk.END).strip()
    if not zawartosc:
        return
    ...
    if sciezka:
        try:
            ...
            messagebox.showinfo("Sukces", "Raport został pomyślnie zapisany!")
        except Exception as e:
            messagebox.showerror("Błąd zapisu", f"Wystąpił problem z zapisem pliku:\n{e}")


# --- INICJALIZACJA OKNA ---
root = tk.Tk()
root.title("Kalkulator Dowódcy Freeciv - Edycja Artyleryjska")
root.geometry("520x900") # Lekko zwiększona wysokość na nowy przycisk
...
btn_oblicz = tk.Button(frame_buttons, text="Uruchom Symulację Dowódczą", command=uruchom_symulacje, bg="#ffcccb", font=("Arial", 10, "bold"), width=25)
btn_oblicz.grid(row=0, column=0, padx=5)

btn_zapisz = tk.Button(frame_buttons, text="Zapisz Raport (TXT)", command=zapisz_raport, bg="#c2f0c2", font=("Arial", 10, "bold"), width=20, state=tk.DISABLED)
btn_zapisz.grid(row=0, column=1, padx=5)
