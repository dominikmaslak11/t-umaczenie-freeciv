...
def uruchom_symulacje():
    try:
        ...
        # Walidacja
        if any(val <= 0 for val in [bazowy_s, bazowy_r, a_hp, d_hp, a_fp, d_fp, a_koszt, d_koszt]):
            messagebox.showerror("Błąd", "Wartości bazowe i koszty muszą być > 0.")
            return
            
        # Obliczanie ostatecznej Siły Ataku i Obrony
        ...
        # --- ANALIZA EKONOMICZNA ---
        if rekomendacja > 0:
            koszt_calkowity_ataku = rekomendacja * a_koszt
            bilans = d_koszt - koszt_calkowity_ataku
            
            text_wynik.insert(tk.END, f"WERDYKT TAKTYCZNY: Potrzebujesz ok. {rekomendacja} jednostek na sukces.\n")
            text_wynik.insert(tk.END, f"WERDYKT EKONOMICZNY:\n")
            text_wynik.insert(tk.END, f" - Koszt twojej fali: {koszt_calkowity_ataku} tarcz\n")
            text_wynik.insert(tk.END, f" - Wartość obrońcy:   {d_koszt} tarcz\n")
            
            if bilans > 0:
                text_wynik.insert(tk.END, f" -> Wymiana BARDZO OPŁACALNA! Zyskujesz {bilans} tarcz przewagi.\n")
            elif bilans > -a_koszt:
                text_wynik.insert(tk.END, f" -> Wymiana wyrównana. Decyduje wartość strategiczna pola.\n")
            else:
                text_wynik.insert(tk.END, f" -> STRATA EKONOMICZNA ({-bilans} tarcz). Atakuj tylko,\n    jeśli to kluczowe miasto lub otwiera drogę!\n")
        else:
...
# --- INICJALIZACJA OKNA ---
root = tk.Tk()
root.title("Kalkulator Taktyczno-Ekonomiczny Freeciv")
root.geometry("520x820")

# --- RAMKA ATAKUJĄCEGO ---
frame_atak = tk.LabelFrame(root, text="Atakujący", padx=10, pady=5)
...
# --- PANEL WYNIKÓW ---
frame_wynik = tk.LabelFrame(root, text="Symulacja Przebiegu Oblężenia i Kosztów", padx=10, pady=5)
frame_wynik.pack(fill="both", expand=True, padx=10, pady=5)

text_wynik = tk.Text(frame_wynik, height=18, width=55, state=tk.DISABLED, bg="#1e1e1e", fg="#00ff00", font=("Consolas", 9))
text_wynik.pack()
